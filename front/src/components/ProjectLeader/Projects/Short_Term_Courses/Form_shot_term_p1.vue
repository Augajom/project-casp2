      ...
      >
        ...
        >
          ...
          <div class="mb-2">
            ...
            <input
              type="text"
              v-model="Project_Data_ST_P1.project_name"
              class="form-control beautiful-box"
              placeholder="โปรดกรอก ชื่อโครงการ"
            />
          </div>

          <div v-for="(item, index) in Project_Data_ST_P1.agency_group" :key="index">
            <hr class="text-white" />
            <div class="mb-3">
              <div class="row">
                <div class="col-9">
          ...
          <div class="d-flex justify-content-center mb-5">
            ...
            >
              ...
              >เพิ่มหน่วยงานร่วมดำเนินโครงการ ({{
                Project_Data_ST_P1.agency_group.length
              }}/6)
            </button>
          </div>

          <div
            v-for="(item, index) in Project_Data_ST_P1.project_manager_leader_lists"
            :key="index"
          >
            <div class="row">
              <div class="col-9">
                ...
                <input
                  type="text"
                  v-model="item.project_manager_leader"
                  class="form-control beautiful-box"
          ...
          <div
            v-for="(item, index) in Project_Data_ST_P1.project_manager_team_lists"
            :key="index"
          >
            <hr class="text-white" />
            <div class="row">
              <div class="col-9">
                <label for="project_name" class="form-label fs-5 text-white"
                  >รายชื่อคณะบริหารโครงการ ลำดับที่ 1</label
                >
                <input
                  type="text"
          ...
          <div class="d-flex justify-content-center mb-3">
            ...
            >
              ...
              >เพิ่มรายชื่อคณะบริหารโครงการ ({{
                Project_Data_ST_P1.project_manager_team_lists.length
              }}/6)
            </button>
          </div>
...
let canEdit = false;
const isCanGo = computed(() => isFormValid.value);

var Project_Data_ST_P1 = reactive({
  project_name: "",
  agency_group: [
    {
      executive_board: "",
...
const submitFormThenNext = async () => {
  ...
  //console.log("ส่งข้อมูล:", JSON.stringify(Project_Data_ST_P1));

  try {
    const response = await axios.post("http://localhost:5000/pl/p_1/shot_term/save", {
      project_id: project_id.value,
      project_type: project_type.value,
      project_user: user_id.value,
      project_data: Project_Data_ST_P1,
...
const getDataWhenProjectID = async () => {
  try {
    const response = await axios.post("http://localhost:5000/pl/ST/get/p_1", {
      project_id: project_id.value,
      project_type: project_type.value,
      project_user: user_id.value,
    });
    ...
    Project_Data_ST_P1.project_name = project_data.project_name;
    Project_Data_ST_P1.agency_group = project_data.agency_group;
    Project_Data_ST_P1.project_manager_leader_lists =
      project_data.project_manager_leader_lists;
    Project_Data_ST_P1.project_manager_team_lists =
      project_data.project_manager_team_lists;
    isCanGo = true;
...
const decrease_agency_group = (index) => {
  if (Project_Data_ST_P1.agency_group.length > 1) {
    Project_Data_ST_P1.agency_group.splice(index, 1);

    if (Project_Data_ST_P1.agency_group.length < agency_group_limits) {
...
const increase_agency_group = () => {
  const totalPercentage = Project_Data_ST_P1.agency_group.reduce((sum, agency) => {
    return sum + (parseFloat(agency.percentage_of_participation) || 0);
  }, 0);

  if (is_agency_full.value) return;

  Project_Data_ST_P1.agency_group.push({
    executive_board: "",
    percentage_of_participation: 0,
    executive_board_leader_name: "",
  });
...
watch(
  () => Project_Data_ST_P1.agency_group.map((a) => a.percentage_of_participation),
  () => {
    const totalPercentage = Project_Data_ST_P1.agency_group.reduce((sum, agency) => {
      return sum + (parseFloat(agency.percentage_of_participation) || 0);
    }, 0);
    ...
    if (
      totalPercentage >= 100 ||
      Project_Data_ST_P1.agency_group.length >= agency_group_limits
    ) {
...
const increase_pm_list = () => {
  if (is_pm_list_full.value) {
    return;
  }
  if (Project_Data_ST_P1.project_manager_team_lists.length >= pm_lists_limits) {
    is_pm_list_full.value = true;
    return;
  }

  Project_Data_ST_P1.project_manager_team_lists.push({
    project_manager_team: "",
    percentage_of_participation: "",
  });
};
const decrease_pm_lists = (index) => {
  Project_Data_ST_P1.project_manager_team_lists.splice(index, 1);

  // คำนวณเปอร์เซ็นต์ใหม่
  const leaderPercent = Project_Data_ST_P1.project_manager_leader_lists.reduce(
    (sum, leader) => sum + (Number(leader.percentage_of_participation) || 0),
    0
  );
  const teamPercent = Project_Data_ST_P1.project_manager_team_lists.reduce(
    (sum, member) => sum + (Number(member.percentage_of_participation) || 0),
    0
  );
  is_pm_list_full.value = leaderPercent + teamPercent >= 100 || Project_Data_ST_P1.project_manager_team_lists.length >= pm_lists_limits;
};


const isFormValid = computed(() => {
  if (!Project_Data_ST_P1.project_name.trim()) return false;

  // เช็ค agency_group
  for (const agency of Project_Data_ST_P1.agency_group) {
    if (
  ...
  for (const member of Project_Data_ST_P1.project_manager_team_lists) {
    if (
...
// เช็ค project_manager_leader_lists (ถ้ามีหลายคน)
for (const leader of Project_Data_ST_P1.project_manager_leader_lists) {
  if (
    Number(leader.percentage_of_participation) < 0
  )
    return false;
}
...
if (leaderPercent + teamPercent !== 100) return false;
  ...
  const agencyTotal = Project_Data_ST_P1.agency_group.reduce((sum, agency) => {
    return sum + (parseFloat(agency.percentage_of_participation) || 0);
  }, 0);
...
watch(
  () => [
    ...Project_Data_ST_P1.project_manager_leader_lists.map(
      (l) => l.percentage_of_participation
    ),
    ...Project_Data_ST_P1.project_manager_team_lists.map(
      (m) => m.percentage_of_participation
    ),
  ],
  () => {
    const leaderPercent = Project_Data_ST_P1.project_manager_leader_lists.reduce(
    ...
    is_pm_list_full.value =
      total >= 100 ||
      Project_Data_ST_P1.project_manager_team_lists.length >= pm_lists_limits;
  },
  { deep: true }
);
