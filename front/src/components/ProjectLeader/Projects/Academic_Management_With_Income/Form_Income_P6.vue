      ...
      <div style="margin-top: 250px">
        ...
        >
          ...
          >
            ...
            <div class="col-auto">
              <router-link
                :to="{ path: '/pl/in_come/p_5', query: { project_id: project_id || '', project_type: 2 } }"
              >
                <button
...
function calculateTotal() {
  ...
  Project_Data_P6.totalFc = totalFc;
  Project_Data_P6.totalVc = totalVc;

  console.log('Vc:' , totalVc)
  console.log('Fc:' , totalFc)
}
...
const getDataWhenProjectID = async () => {
  try {
    ...
    Project_Data_P6.totalFc = project_data.totalFc;
    Project_Data_P6.totalVc = project_data.totalVc;
...
const saveData = debounce(async () => {
  try {
    const response = await axios.post("http://localhost:5000/pl/p_6/income/save", {
      project_id: project_id.value,
      project_type: project_type.value,
      project_user: user_id.value,
...
watch(
  ...
  (newVal) => {
    ...
    // ตรวจสอบแต่ละรายการตามเงื่อนไขของ calculateTotal()
    const checkUnits = (items, name) => {
    ...
    // ตรวจสอบทุกกลุ่ม
    checkUnits(newVal.compensation.selectedCompensations, "Compensation");
    checkUnits(newVal.foodAccommodation.selectedFoodAccommodation, "FoodAccommodation");
    checkUnits(newVal.otherExpenses.selectedOtherExpenses, "OtherExpenses");
