...
const autoSave = debounce(async () => {
  // loading.value = true
  try {
    const response = await axios.post("http://localhost:5000/pl/p_4/no_income/save", {
      project_id: project_id.value,
      project_type: project_type.value,
      project_user: user_id.value,
...
watch(
  ...
  (newVal) => {
    ...
    if (!Array.isArray(newVal.multipleSessions)) {
      //console.log("❌ multipleSessions is not an array");
      isCanGo = true;
      return;
    }
    for (let i = 0; i < newVal.multipleSessions.length; i++) {
      const session = newVal.multipleSessions[i];
      if (!session.location?.trim()) {
        //console.log(`❌ multipleSessions[${i}].location is empty`);
        isCanGo = true;
        return;
      }
      if (!session.date?.trim()) {
        //console.log(`❌ multipleSessions[${i}].date is empty`);
        isCanGo = true;
        return;
      }
      //console.log(`✅ multipleSessions[${i}] is complete`);
    }
