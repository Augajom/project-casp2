...
const Save_TS_P7 = (data, callback) => {
    ...
    const rawProjectData = typeof project_data === 'string' ? JSON.parse(project_data) : project_data;

    const projectData = JSON.stringify({
    targetGroupCount: rawProjectData.target_group_amount,
    costPerPerson: rawProjectData.cost_per_person,
    totalCost: rawProjectData.net_total_cost,
    });
    
    if (!rawProjectData || Object.keys(rawProjectData).length === 0) {
        console.warn('Empty project_data:', rawProjectData);
        }
