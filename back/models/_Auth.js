...
const googleAuth = async (data, callback) => {
  ...
  if (!user || user.length === 0) {
    return callback({ message: "Email นี้ไม่มีในระบบ" }, null);
  }
  ...
  if (formatuser[0]) {
    ...
    const token = await genToken({
      email: DataSQL.user_email,
      roles: DataSQL.roles,
      user_id: DataSQL.user_id,
      affiliation_id: DataSQL.affiliation_id,
      affiliation_name: DataSQL.affiliation_name,
      isAuthen: true,
    });
    callback(null, token);
  } else {
